...
?>
    ...
    <form  class="edit-form" method="POST" action="<?php echo INCLUDE_PATH_PAINEL?>pages/ajax_post.php?id=<?php echo $id ?>&event=<?php echo $value->event_post?>">
        <?php 
        ...
        <label>Autor:</label> 
        <input  type='text' name='autor_post' value='<?php echo $value->autor_post; ?>' required>
        <label>Instrutor:</label> 
        <select name='instrutor_post' required>";
        <option selected value="<?php echo $value->autor_post?>"><?php echo $value->autor_post; ?></option>
    <?php 
        }  
    ...
    <?php  if ( $value->status_post != 'aguardando' ): ?> 
                ...
                <input type='date' name='date_post' value='<?php echo $value->date_post; ?>' >
                <span class="obs"> 
