...
class Crud{
  // crud posts
    ...
    protected static function readPost ( $cond ) {
        $sql = Mysql::conectar()->prepare( " SELECT * FROM posts $cond ORDER BY id_post ASC " );
        if( $sql->execute() )
        {
            if ( ($sql->rowCount() ) == 0 )
            {
    ...
    protected static function createEvent ( $nome,$data,$id) {
        $sql = Mysql::conectar()->prepare( " CREATE EVENT IF NOT EXISTS $nome ON SCHEDULE AT TIMESTAMP '$data 07:00:00' DO UPDATE posts SET status_post='concluido' WHERE id_post=$id  " );
        if ( $sql->execute ( ) )
        {
            return true;
        }
